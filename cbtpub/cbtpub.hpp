...
const account_name MYWALLET = N(mywallet);
const symbol_name CBT_SYMBOL_NAME = SN(CBT);
const symbol_type CBT_SYMBOL = S(4, CBT);
const asset NEW_GAME_CONSOME = asset(1000000, CBT_SYMBOL);
const uint32_t MAX_PERIOD = 100ul * 365 * 24 * 60 * 60;

class cbtpub: public contract {
public:
    cbtpub(account_name self):
            contract(self)
    {};
...
private:
    ...
    //         N(eosio.token),
    //         N(transfer),
    //         make_tuple(_self, MYWALLET, quantity * 85 / 100, string("cbtpub reserve https://dapp.pub"))
    //     ).send();
    // }
    ...
    struct st_game {
        ...
        uint8_t _fee_percent() {
            print(" ... _fee_percent: ", uint32_t(init_fee_percent), " ", uint32_t(base_fee_percent));
            ...
            auto ret =  uint8_t(ceil(2 * PERIOD * (INIT - BASE) / ((NOW-START) + PERIOD) + 2 * BASE - INIT));
            print(" ret:", uint32_t(ret));
            return ret;
        }

        void _profit_eos(int64_t eos_amount) {
            print(" >>> _profit_eos: ", eos_amount);
            ...
            print(" BASE_STAKE:", BASE_STAKE);
            print(" BASE_EOS:", BASE_EOS);
            print(" STAKE:", STAKE);
            print(" EOS:", EOS);
            print(" PROFIT:", PROFIT);
            
            eos = (EOS - BASE_EOS + PROFIT) * BASE_STAKE / (BASE_STAKE - STAKE);
            base_eos = (EOS - BASE_EOS + PROFIT) * STAKE / (BASE_STAKE - STAKE);
            print(" eos:", eos, " base_eos:", base_eos);
        }

        void _issue_stake(int64_t stake_amount) {
            print(" >>> _issue_stake: ", stake_amount);
            ...
            print(" STAKE:", STAKE);
            print(" EOS:", EOS);
            print(" NEW_STAKE:", NEW_STAKE);
            eos = (BASE_STAKE + NEW_STAKE) * (EOS - BASE_EOS) / (BASE_STAKE + NEW_STAKE - STAKE);
            base_eos = (STAKE) * (EOS - BASE_EOS) / (BASE_STAKE + NEW_STAKE - STAKE);
            base_stake += stake_amount;

            print(" eos:", eos, " base_eos:", base_eos, " base_stake:", base_stake);
        }

        void _consume_stake(int64_t stake_amount) {
            print(" >>> _consume_stake: ", stake_amount);
            ...
            eos = (EOS - BASE_EOS) * BASE_STAKE / (BASE_STAKE - STAKE - CONSUME);
            base_eos = (EOS - BASE_EOS) * (STAKE + CONSUME) / (BASE_STAKE - STAKE - CONSUME);
            stake += stake_amount;

            print(" eos:", eos, " base_eos:", base_eos, " stake:", stake);
        }

        void _update_option() {
            print(" >>> _update_option: ", base_option, " ", deserved_option);
            ...
            print(" new_deserved_option: ", new_deserved_option);
        ...
        int64_t fee(int64_t eos_amount) {
            print(" >>> fee eos: ", eos_amount);
            int64_t fee = 0;
            int64_t fee_percent = _fee_percent();
            print(" ... fee per: ", fee_percent);
    ...
    asset game_buy(symbol_name name, int64_t eos_amount) {
        ...
        print(" >>> game_buy stake_out:", stake_amount);
    ...
    tuple<asset, asset> game_sell(symbol_name name, int64_t stake_amount) {
        ...
        print(" >>> game_sell eos_out:", eos_amount);
...
#ifdef ABIGEN
    EOSIO_ABI(cbtpub, (settrans)(setreferfee)(issue)(create)(receipt)(transfer)(sell)(consume)(destroy)(claim)(newtoken))
#endif